...
#Questions----------------------------------------------------------------------------
#Is there a relationship between duration of visit and number of animals at a species’ preferred watering hole type?
#Is there a relationship between duration of visit and season at a species’ preferred watering hole type? 
#How do these relationships compare between the three tracked species?
...
#Figure-Elephants-NoAnimals-Duration==========================================================
#Elephant Mod2
#prep predict data with CI
predict_eleQ1mod2 <-data.frame(NoAnimals=seq(0, max(ele_res_data_clean$NoAnimals),
                                             by=0.5))
predict_eleQ1mod2_se <-predict(eleQ1mod2,
                               newdata=predict_eleQ1mod2,
                               se.fit=TRUE,
                               type="link")
predict_eleQ1mod2$fit<-exp(predict_eleQ1mod2_se$fit)
predict_eleQ1mod2$lower<-exp(predict_eleQ1mod2_se$fit-1.96*predict_eleQ1mod2_se$se.fit)
predict_eleQ1mod2$upper<-exp(predict_eleQ1mod2_se$fit+1.96*predict_eleQ1mod2_se$se.fit)
#figure
ggplot()+
  #shaded (CI)
  geom_ribbon(data=predict_eleQ1mod2,
              aes(x=NoAnimals,ymin=lower,ymax=upper),
              fill="steelblue",alpha=0.3)+
  #original data point
  geom_point(data=ele_res_data_clean,
             aes(x=NoAnimals,y=DurationMins),
             alpha=0.5,size=2,color="gray30")+
  #predict line
  geom_line(data=predict_eleQ1mod2,
            aes(x=NoAnimals,y=fit),
            color="steelblue",size=1.2)+
  #labels
  labs(title="African Savanna Elephant - Effects of Number of Animals on Duration of Visit",
       subtitle="Gamma GLM with 95% confidence intervals",
       x="Number of Animals",
       y="Duration(minutes)",
       caption="African Savanna Elephant Model 2 - Each additional animal increases duration by 22.9%")+
  #theme
  theme_minimal()+
...
#Figure-Elephant-NoAnimals-Duration-Season-----------------------------------------
#ElephantMod3
predict_eleQ1mod3 <-expand.grid(
  NoAnimals=seq(0, max(ele_res_data_clean$NoAnimals),by=0.5),
  Season=unique(ele_res_data_clean$Season)
  )
predict_eleQ1mod3_se <-predict(eleQ1mod3,
                               newdata=predict_eleQ1mod3,
                               se.fit=TRUE,
                               type="link")
predict_eleQ1mod3$fit<-exp(predict_eleQ1mod3_se$fit)
predict_eleQ1mod3$lower<-exp(predict_eleQ1mod3_se$fit-1.96*predict_eleQ1mod3_se$se.fit)
predict_eleQ1mod3$upper<-exp(predict_eleQ1mod3_se$fit+1.96*predict_eleQ1mod3_se$se.fit)
#figure
ggplot()+
  #shaded (CI)
  geom_ribbon(data=predict_eleQ1mod3,
              aes(x=NoAnimals,ymin=lower,ymax=upper,fill=Season),
              alpha=0.3)+
  #original data point (colored by season)
  geom_point(data=ele_res_data_clean,
             aes(x=NoAnimals,y=DurationMins, color=Season),
             alpha=0.5,size=2)+
  #predict line (one per season)
  geom_line(data=predict_eleQ1mod3,
            aes(x=NoAnimals,y=fit, color=Season),
            size=1.2)+
  #labels
  labs(title="African Savanna Elephant - Effects of Number of Animals on Duration of Visit by Season",
       subtitle="Gamma GLM with 95% confidence intervals",
       x="Number of Animals",
       y="Duration(minutes)",
       caption="African Savanna Elephant Model 3
       Season 1 = Warm/Wet Season
       Season 2 = Cold/Dry Season 
       Visits during Season 2 were 39% longer than visits during Season 1") +
  #theme
  theme_minimal()+
  theme(plot.title=element_text(size=14,face="bold"),
        plot.subtitle=element_text(size=11),
...
#Figure-Black-Rhino-NoAnimals-Duration==========================================================
#Black Rhino Mod2
predict_blkrhnQ1mod2 <-data.frame(NoAnimals=seq(0, max(blkrhn_earthdam_data_clean$NoAnimals),
                                             by=0.5))
predict_blkrhnQ1mod2_se <-predict(blkrhnQ1mod2,
                               newdata=predict_blkrhnQ1mod2,
                               se.fit=TRUE,
                               type="link")
predict_blkrhnQ1mod2$fit<-exp(predict_blkrhnQ1mod2_se$fit)
predict_blkrhnQ1mod2$lower<-exp(predict_blkrhnQ1mod2_se$fit-1.96*predict_blkrhnQ1mod2_se$se.fit)
predict_blkrhnQ1mod2$upper<-exp(predict_blkrhnQ1mod2_se$fit+1.96*predict_blkrhnQ1mod2_se$se.fit)
#figure
ggplot()+
  #shaded (CI)
  geom_ribbon(data=predict_blkrhnQ1mod2,
              aes(x=NoAnimals,ymin=lower,ymax=upper),
              fill="steelblue",alpha=0.3)+
  #original data point
  geom_point(data=blkrhn_earthdam_data_clean,
             aes(x=NoAnimals,y=DurationMins),
             alpha=0.5,size=2,color="gray30")+
  #predict line
  geom_line(data=predict_blkrhnQ1mod2,
            aes(x=NoAnimals,y=fit),
            color="steelblue",size=1.2)+
  #labels
  labs(title="Black Rhino - Effects of Number of Animals on Duration of Visit",
       subtitle="Gamma GLM with 95% confidence intervals",
       x="Number of Animals",
       y="Duration(minutes)",
       caption="Black Rhino Model 2 - Each additional animal increases duration by 174.3%")+
  #theme
  theme_minimal()+
  theme(plot.title=element_text(size=14,face="bold"),
        plot.subtitle=element_text(size=11),
...
#Figure-Black-Rhino-NoAnimals-Duration-Season-----------------------------------------
#BlackRhinoMod3
predict_blkrhnQ1mod3 <-expand.grid(
  NoAnimals=seq(0, max(blkrhn_earthdam_data_clean$NoAnimals),by=0.5),
  Season=unique(blkrhn_earthdam_data_clean$Season)
)
predict_blkrhnQ1mod3_se <-predict(blkrhnQ1mod3,
                               newdata=predict_blkrhnQ1mod3,
                               se.fit=TRUE,
                               type="link")
predict_blkrhnQ1mod3$fit<-exp(predict_blkrhnQ1mod3_se$fit)
predict_blkrhnQ1mod3$lower<-exp(predict_blkrhnQ1mod3_se$fit-1.96*predict_blkrhnQ1mod3_se$se.fit)
predict_blkrhnQ1mod3$upper<-exp(predict_blkrhnQ1mod3_se$fit+1.96*predict_blkrhnQ1mod3_se$se.fit)
#figure
ggplot()+
  #shaded (CI)
  geom_ribbon(data=predict_blkrhnQ1mod3,
              aes(x=NoAnimals,ymin=lower,ymax=upper,fill=Season),
              alpha=0.3)+
  #original data point (colored by season)
  geom_point(data=blkrhn_earthdam_data_clean,
             ...
             alpha=0.5,size=2)+
  #predict line (one per season)
  geom_line(data=predict_blkrhnQ1mod3,
            aes(x=NoAnimals,y=fit, color=Season),
            size=1.2)+
  #labels
  labs(title="Black Rhino - Effects of Number of Animals on Duration of Visit by Season",
       subtitle="Gamma GLM with 95% confidence intervals",
       x="Number of Animals",
       y="Duration(minutes)",
       caption="Black Rhino Model 3
       Season 1 = Warm/Wet Season
       Season 2 = Cold/Dry Season 
       Visits during Season 2 were 29% shorter than visits during Season 1") +
  #theme
  theme_minimal()+
  theme(plot.title=element_text(size=14,face="bold"),
        plot.subtitle=element_text(size=11),
...
#Statistical-Tests__________________________________________________________________
anova(eleQ1mod2,eleQ1mod3,test="Chisq")
anova(blkrhnQ1mod2,blkrhnQ1mod3,test="Chisq")
#citations -----------------------------------------------------------------------
citation("readxl")
citation("lubridate")
citation("dplyr")
